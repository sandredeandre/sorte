...
$result = "SELECT * FROM winner  LEFT JOIN user ON
 (winner.id_user= user.id) WHERE winner.id_user = user.id  && tipoinst = 3";
...
<div class="col-4">
	...
	<?php 
 $id_user=$_SESSION["id_user"];
 	$result = "SELECT id_user COUNT FROM winbackup WHERE  tipoinst = 3";
		$resultado = mysqli_query($conn, $result);
...
<?php
 ...
 (winbackup.id_user= user.id) WHERE winbackup.id_user = user.id && winbackup.tipoinst = 3";
  $resultado = mysqli_query($conn, $result);
...
<?php
 ...
 <img  class="img  rounded-3 img-thumbnail" src="../img/pinst2.png"  >
<div class="dropdown-divider"></div>
