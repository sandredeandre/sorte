...
$result = "SELECT * FROM winner  LEFT JOIN user ON
 (winner.id_user= user.id) WHERE winner.id_user = user.id  && tipoinst = 8";
...
<div class="col-4">
	...
	<?php 
 $id_user=$_SESSION["id_user"];
 	$result = "SELECT id_user COUNT FROM winbackup WHERE  tipoinst = 8";
		$resultado = mysqli_query($conn, $result);
		$resulta = mysqli_num_rows($resultado);?>
	<div class="text-center">
	<h6 class="display-6 text-dark"><?php echo $resulta; ?></h6></div>
	<div class="dropdown-divider"></div>
		<img width="80%" class="rounded-circle img-thumbnail" src="../../img/prem.png" >	
	<div class="dropdown-divider"></div>	
<?php
	if (isset($resulta)) {
		$result = "SELECT * FROM winbackup  LEFT JOIN user ON
 (winbackup.id_user= user.id) WHERE winbackup.id_user = user.id && winbackup.tipoinst = 8";
  $resultado = mysqli_query($conn, $result);
while ($resulta = mysqli_fetch_assoc($resultado)){;
			$img= base64_decode($resulta['img'])
			?>
		
<img width="18%" class="rounded-circle img-thumbnail" src="../../img/<?php echo $img;?>" >		
