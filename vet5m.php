...
$result = "SELECT * FROM winner  LEFT JOIN user ON
 (winner.id_user= user.id) WHERE winner.id_user = user.id  && tipoinst = 6";
...
<div class="col-4">
	...
	<?php 
 $id_user=$_SESSION["id_user"];
 	$result = "SELECT id_user COUNT FROM winbackup WHERE  tipoinst = 6";
		$resultado = mysqli_query($conn, $result);
		$resulta = mysqli_num_rows($resultado);?>
	<div class="text-center">
	<h6 class="display-6 text-dark"><?php echo $resulta; ?></h6></div>
	<div class="dropdown-divider"></div>
		<img width="60%" class="rounded-circle img-thumbnail" src="../img/prem.png" >	
	<div class="dropdown-divider"></div>	
<?php
	if (isset($resulta)) {
		$result = "SELECT * FROM winbackup  LEFT JOIN user ON
 (winbackup.id_user= user.id) WHERE winbackup.id_user = user.id && winbackup.tipoinst = 6";
  $resultado = mysqli_query($conn, $result);
while ($resulta = mysqli_fetch_assoc($resultado)){;
			$img= base64_decode($resulta['img'])
			?>
		
<img width="16%" class="rounded-circle img-thumbnail" src="../img/<?php echo $img;?>" >		
...
<?php
 ...
 <img  class="img  rounded-3 img-thumbnail" src="../img/pinst5.png"  >
<div class="dropdown-divider"></div>
